...
- (id)initWithDelegate:(NSObject<SuperpoweredIOSAudioIODelegate> *)d preferredBufferSize:(unsigned int)preferredBufferSize preferredMinimumSamplerate:(unsigned int)prefsamplerate audioSessionCategory:(NSString *)name channels:(int)channels audioProcessingCallback:(audioProcessingCallback)callback clientdata:(void *)clientdata {
    self = [super init];
    if (self) {
        ...
        audioSessionCategory = [name retain];
#else
        audioSessionCategory = name;
#endif
        ...
        bool recordOnly = [name isEqualToString:AVAudioSessionCategoryRecord];
        inputEnabled = recordOnly || [name isEqualToString:AVAudioSessionCategoryPlayAndRecord];
#else
...
- (void)onRouteChange:(NSNotification *)notification {
    ...
    for (AVAudioSessionPortDescription *port in [[[AVAudioSession sharedInstance] currentRoute] outputs]) {
        ...
        audioDeviceType ___class = NSStringToAudioDeviceType(port.portType);
        ...
        if (___class == audioDeviceType_headphone) outputChannelMap.headphoneAvailable = true;
        else if (___class == audioDeviceType_HDMI) {
...
#endif
        } else if (___class == audioDeviceType_USB) { // iOS can handle one USB audio device only
        ...
        while (channels > 0) {
            RemoteIOOutputChannelMap[n++] = ___class;
            channels--;
        };
    };
    ...
    if ([[AVAudioSession sharedInstance] isInputAvailable]) {
        ...
        for (AVAudioSessionPortDescription *port in [[[AVAudioSession sharedInstance] currentRoute] inputs]) {
            ...
            first = false;
            if (___class == audioDeviceType_USB) inputChannelMap.numberOfUSBChannelsAvailable = channels;
        };
...
- (void)reconfigureWithAudioSessionCategory:(NSString *)name {
    if (![NSThread isMainThread]) {
        [self performSelectorOnMainThread:@selector(reconfigureWithAudioSessionCategory:) withObject:name waitUntilDone:NO];
        return;
    };
#if !__has_feature(objc_arc)
    [audioSessionCategory release], audioSessionCategory = [name retain];
#else
    audioSessionCategory = name;
#endif
